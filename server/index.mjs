...
async function serveStatic(req, res, pathname) {
  if (!fs.existsSync(DIST)) {
    ...
    res.end(
      `<!doctype html><meta charset="utf-8"><title>Pansari</title>
       ...
       <div><h1>Pansari is not built yet</h1>
...
const server = http.createServer(async (req, res) => {
  ...
  if (authEnabled() && pathname !== '/api/health') {
    if (!isConfigured()) {
      send(res, 401, { error: 'Set a passcode before using Pansari.', code: 'setup_required' });
      return;
    }
  ...
  try {
    ...
    send(res, 200, result, download
      ? { 'content-disposition': `attachment; filename="pansari-backup-${new Date().toISOString().slice(0, 10)}.json"` }
      : {});
  } catch (err) {
...
if (nodeMajor < MIN_NODE_MAJOR) {
  console.error(`\n  Pansari needs Node ${MIN_NODE_MAJOR} or newer.`);
  console.error(`  This computer has Node ${process.versions.node}.`);
  console.error('  Install the current LTS from https://nodejs.org and start Pansari again.\n');
  process.exit(1);
}
...
server.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    ...
    console.error('  Pansari is probably already running — check your browser first.');
...
try {
  ...
  console.error('\n  Pansari could not open its data file.');
...
for (const fatal of ['uncaughtException', 'unhandledRejection']) {
  process.on(fatal, (err) => {
    console.error(`\n[fatal] ${fatal}:`, err);
    console.error('[fatal] Shutting down; the service manager should restart Pansari.\n');
    process.exit(1);
  });
}
