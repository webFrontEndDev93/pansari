...
const PREFIX = 'pansari-';

const isBackup = (name) => name.endsWith('.json') && name.startsWith(PREFIX);

// Sort on the timestamp rather than the whole filename. With one prefix the two
// are equivalent, but if this product is ever renamed the new prefix may sort
// before the old one whatever the dates say — which would have pruning keep the
// oldest backups and delete the newest.
const stampOf = (name) => name.slice(PREFIX.length);
