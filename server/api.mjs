 ...
 * honest: it can only build a product or a stock lot the same way the ordinary
 * Add Item and Receive Stock routes do.
 */
...
export const routes = [
  ...
  // The validators the import reuses, so a bulk row cannot get past anything a
  // typed-in one could not.
  ['POST', '/api/import/preview', (_p, body) => {
    ...
    return {
      ...
      failed: plan.failed,
      rows: plan.plans.map(({ productPayload, batchPayload, ...rest }) => rest),
    };
  }, 'admin'],
  ...
  ['POST', '/api/restore', async (_p, body) => {
    if (!body || !Array.isArray(body.products)) throw bad('That file does not look like a Pansari backup.');
