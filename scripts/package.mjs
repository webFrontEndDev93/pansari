...
const out = path.join(root, 'pansari-shop');
...
await fsp.writeFile(
  ...
  [
    '@echo off',
    'REM Sets up Pansari: checks Node, puts an icon on the desktop.',
...
await fsp.writeFile(
  ...
  [
    ...
    '',
    '  It puts a Pansari icon on the desktop and offers to start the till',
    '  automatically whenever the computer is switched on.',
    '',
    '',
    'THEN — day to day',
    '',
    '  Double-click the Pansari icon on the desktop. The first time, it asks',
    ...
    '',
    '  Pansari starts with about 65 common karyana items already listed, so you',
    '  are not typing into an empty screen. Each one has a price but no stock.',
    '  Roughly half are sold by weight — atta, chawal, daal, masalay, sabzi —',
    '  and those are priced per kilo.',
    '',
    '  Open Inventory and, for every item you actually keep, press Stock and',
    '  enter what is on the shelf: the weight in the sack, or how many packets.',
    '  Fix the price at the same time. For anything with a date on the carton,',
    '  put that in too. Delete the items you do not sell, and add the ones that',
    '  are missing.',
    '',
