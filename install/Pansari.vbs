...
If Not ServerUp() Then
  ...
  If Err.Number <> 0 Then
    On Error GoTo 0
    MsgBox "Pansari needs Node.js, which does not seem to be installed." & vbCrLf & vbCrLf & _
           "Install it once from https://nodejs.org (choose the LTS version)," & vbCrLf & _
           "then open Pansari again.", 48, "Pansari"
    WScript.Quit 1
  End If
  On Error GoTo 0
...
If Not ServerUp() Then
  MsgBox "Pansari did not start." & vbCrLf & vbCrLf & _
         "Try restarting the computer. If it still will not open, the shop's" & vbCrLf & _
         "records are safe in:" & vbCrLf & appDir & "\server\data", 16, "Pansari"
  WScript.Quit 1
End If
