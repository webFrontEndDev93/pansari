#!/usr/bin/env bash
# Starts Pansari if it is not already running, then opens it in its own window.
...
LOG="${APP_DIR}/server/data/pansari.log"

up() { curl -fsS --max-time 2 "${URL}/api/health" >/dev/null 2>&1; }

notify() {
  # Whatever this desktop has: a dialog, a notification, or just the terminal.
  if command -v zenity >/dev/null 2>&1; then zenity --error --title=Pansari --text="$1" 2>/dev/null
  elif command -v osascript >/dev/null 2>&1; then osascript -e "display alert \"Pansari\" message \"$1\"" >/dev/null 2>&1
  else echo "$1" >&2
  fi
}

if ! command -v node >/dev/null 2>&1; then
  notify "Pansari needs Node.js, which is not installed. Install it once from https://nodejs.org, then open Pansari again."
  exit 1
fi
...
if ! up; then
  notify "Pansari did not start. See ${LOG}. The shop's records are safe in ${APP_DIR}/server/data."
  exit 1
fi
