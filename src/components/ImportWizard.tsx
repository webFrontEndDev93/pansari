...
import { unitKeys, unitOf } from '../lib/units';
...
const FIELDS: ImportField[] = [
  { key: 'name', label: 'Item name', required: true, aliases: ['name', 'item', 'item name', 'product', 'product name', 'description', 'particulars', 'title'] },
  { key: 'urduName', label: 'Urdu name', aliases: ['urdu', 'urdu name', 'nam'] },
  { key: 'brand', label: 'Brand', aliases: ['brand', 'company', 'manufacturer', 'make'] },
  { key: 'category', label: 'Category', aliases: ['category', 'type', 'group', 'department'] },
  { key: 'unit', label: 'Sold by', hint: 'kg, litre, piece, packet or dozen', aliases: ['unit', 'uom', 'sold by', 'measure'] },
  { key: 'size', label: 'Pack size', aliases: ['size', 'pack', 'pack size', 'packing', 'weight'] },
  { key: 'salePrice', label: 'Sale price', hint: 'Per kg or litre for loose goods', aliases: ['price', 'rate', 'sale price', 'selling price', 'unit price', 'retail'] },
  { key: 'costPrice', label: 'Purchase cost', aliases: ['cost', 'cost price', 'purchase', 'purchase price', 'buying'] },
  { key: 'mrp', label: 'Printed MRP', aliases: ['mrp', 'printed price', 'list price'] },
  { key: 'quantity', label: 'Quantity in stock', aliases: ['qty', 'quantity', 'stock', 'count', 'balance', 'on hand'] },
  { key: 'expiry', label: 'Expiry date', aliases: ['expiry', 'exp', 'expiry date', 'best before', 'use by'] },
  { key: 'batchNo', label: 'Batch number', aliases: ['batch', 'batch no', 'lot', 'lot no'] },
  ...
  { key: 'taxRate', label: 'Tax %', aliases: ['tax', 'tax %', 'tax rate', 'gst', 'sales tax', 'vat'] },
  { key: 'aisle', label: 'Aisle / shelf', aliases: ['aisle', 'shelf', 'rack', 'location'] },
    ...
    >
      {!sheet && (
        <div className="import-drop">
          ...
          <p className="muted">
            One row per item. It does not need our column names — you will match them up next.
          </p>
...
const fmt = (qty: number, unit: string) => `${qty} ${unitOf(unit).short}`;
...
function downloadTemplate() {
  ...
  const example = [
    'Atta Chakki Fresh', 'آٹا', '', 'Staples', 'kg', 'Loose',
    '132.50', '112.00', '', '25.5', '', '', 'Akbari Mandi', '', '0', 'C2', '5',
  ];
  const second = [
    'Tapal Danedar', 'چائے', 'Tapal', 'Tea & Beverages', 'piece', '950 g',
    '1460', '1240', '1520', '12', '2027-06-30', 'TAP2417', 'Metro', '8960001234567', '18', 'A1', '6',
  ];
  ...
  link.href = url;
  link.download = 'pansari-stock-template.csv';
  link.click();
  URL.revokeObjectURL(url);
}

/** Units are listed in the template hint so the sheet can use the right words. */
export const IMPORT_UNIT_HINT = () => unitKeys().join(', ');
