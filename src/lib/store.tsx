...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [theme, setTheme] = useState<Theme>(() => {
    const saved = localStorage.getItem('pansari.theme');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('pansari.theme', theme);
  }, [theme]);
  ...
  const checkLock = useCallback(async () => {
    try {
      ...
      setLoadError(error instanceof Error ? error.message : 'Could not reach the Pansari server.');
      setLock('open');
    } finally {
      setReady(true);
    }
